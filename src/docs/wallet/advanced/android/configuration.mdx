...
Beacon Android SDK is not a stateless library and bases part of its functionality on preserved data. By default the SDK stores its data in the [`SharedPreferences`](https://developer.android.com/reference/android/content/SharedPreferences). However, it is possible to override the default storage and provide own implementation. The `:core` module ships with 3 public interfaces that can be satisfied and provided externally during the `BeaconWalletClient` setup: `Storage`, `ExtendedStorage` (an extended version of `Storage`) and `SecureStorage`.
...
Some modules may require additional data to store. Those modules usually come with their own storage plugins which interfaces can be also satisfied and provided externally.
